...
- 이번 프로젝트는 Amazon 제품·가격·사용자·리뷰 데이터셋을 기반으로 전체 데이터 품질을 진단하고 ETL 오류로 인해 손실된 리뷰 정보를 복원하여 신뢰 가능한 분석 체계를 구축하는 것이 목적이다.
- 처음 주어진 데이터는 겉보기와 달리 여러 구조적 문제를 품고 있었다. 특히 Reviews 테이블이 사용자 수와 동일한 개수의 리뷰만 포함하는 등 현실적인 분석이 불가능한 패턴이 드러났다. 이 프로젝트는 그 문제를 찾아내고 해결한 뒤 복원된 데이터로 상품·고객·시장 관점의 다차원 분석을 수행한 과정을 담고 있다.
...
### 2. 데이터 구조 및 문제 진단 (Data Structure & Initial Diagnostics)
...
- Pricing: 가격·평점·할인율
- Users: 사용자 정보
- Reviews (문제 발견): 리뷰 데이터(테이블이 비정상적으로 축소됨)

#### 2.2 주요 문제 발견

- Products 및 Users 테이블은 무결성 양호
- 하지만 Reviews 테이블은 user 수 = review 수 → 유저당 리뷰 1개만 남는 데이터 손실 현상
- raw_data를 조회한 결과 실제로는 5~10개의 리뷰를 남긴 헤비 유저 존재
- 즉 ETL 중 UNIQUE 제약조건을 지키는 과정에서 멀티 리뷰 정보가 사라진 문제임을 확인
...
#### 2.4 테이블 무결성 및 기본 통계 분석

##### 0-1. Products 데이블 데이터 무결성 및 수량 확인
```sql
/*
...
- 이는 테이블의 기본 키(Primary Key) 무결성이 확보되었음을 의미하며 이후 분석 과정에서 다른 테이블과 JOIN 작업 시 데이터 손실이나 중복 없이 신뢰할 수 있는 결합을 수행할 수 있음을 보여준다.

##### 0-2. Pricing 테이블 핵심 통계량 요약
```sql
/*
  분석 쿼리 0-2: 핵심 통계량 요약
...
- 이는 User 한사람이 리뷰를 하나씩 작성했다는 의미를 가지는데 테이블이 데이터를 바르게 반영하고 있는지 확인하기 위하여 추후 점검이 필요해 보인다.

### 3. 문제 해결 및 데이터 복원 (Data Reconstruction)

#### 3.1 해결 전략

기존 Reviews 테이블이 자연키(review_id)를 PK로 사용하다가 충돌이 발생하면서 다중 리뷰가 로딩되지 않은 것이 원인이었다. 이를 해결하기 위해 아래 5가지 전략을 사용하였다.

- 기존 Reviews 테이블 삭제
- Artificial Primary Key (review_pk_id) 도입
- review_id는 일반 컬럼으로 유지하여 충돌 방지
- raw_data에서 모든 리뷰 행을 다시 적재
- 외래키(product_id, user_id)는 정상적으로 매칭

#### 3.2 복원 결과

- 리뷰 데이터가 user당 1건에서 실제 값(5~10건 등)으로 복원됨
- 파워 리뷰어(고가치 고객) 데이터 확보
- 더 이상 UNIQUE constraint 오류 발생하지 않음
- 이후 분석의 신뢰도 확보
...
- 문제는 raw_data 자체에 멀티 리뷰 사용자가 없었던 것이 아니라 raw_data에서 Reviews 테이블로 데이터를 로드하는 과정에서 멀티 리뷰 정보가 모두 손실되었다는 것이다.
...
### 4. 분석 과정 (Exploratory Data Analysis with SQL)
#### 4.1 데이터 품질 분석

##### 1.Pricing 테이블 데이터 완성도 검증 (NULL 값 존재 여부 확인)
```sql
/*
...
### 5. 결론 및 발전 가능성 (Conclusion & Future Work)

#### 5.1 결론
- 처음 데이터셋은 겉으로는 제품·유저·리뷰가 모두 갖추어진 완전한 구조처럼 보였지만 실제로는 리뷰 손실이라는 핵심적 결함이 숨어 있었다. 이 결함을 해결하고 나서야 데이터는 비로소 현실 행동 패턴을 반영하기 시작했다.
고객의 리뷰 생성 패턴, 카테고리 간 가격 구조, 할인 전략의 반응성, 브랜드별 시장 포지션 모두 ‘복원된 데이터’ 위에서야 의미 있는 형태로 드러났다.
데이터를 다시 살려놓고 전체를 살펴보면 몇 가지 큰 흐름이 보인다.
고객 참여에는 명확한 파레토 구조가 존재하며 일부 파워 리뷰어와 일부 핵심 상품군이 전체 생태계를 주도한다. 중간평점대(3.0–3.9)는 개선 시 긍정평점으로 전환 가능한 전략적 타깃으로 활용가능하며 반대로 리뷰 수는 많지만 평점이 낮게 유지되는 제품군은 카테고리의 건전성을 해치는 위험 요소가 될 수 있다. 할인율·평점·리뷰 참여도를 함께 보면 카테고리마다 서로 다른 시장 전략이 통하고 있으며 어떤 상품군은 가격보다 브랜드 신뢰가 어떤 상품군은 할인율보다 실제 사용 경험이 크게 작동한다.

- 이번 프로젝트는 단순히 데이터를 분석했다기보다 데이터 품질 문제를 발견하고 그것을 해결한 뒤 정상화된 데이터로 시장·고객·제품의 구조를 해석했다는 점에서 의미가 있다. 신뢰도 높은 데이터를 확보하는 일이 얼마나 중요한지 그리고 그 신뢰도가 확보되었을 때 비로소 어떤 인사이트들이 살아나는지를 명확히 보여준다.

- 그 결과로 마련된 인사이트는 단순 숫자 관찰이 아니라 제품 개선, 고객 관리, 카테고리 전략, 가격 정책 같은 실제 의사결정에 연결 가능한 형태로 정제되었다.

#### 5.2 향후 발전 가능성

##### 1. 리뷰 텍스트 NLP 분석

- Python을 활용하여 리뷰 텍스트를 단순 문자열에서 의미를 가진 데이터로 전환

    - Python: 분석 전체의 기반
    - spaCy 또는 NLTK: 토큰화, 키워드 추출 같은 전처리
...
- Python의 머신러닝 도구를 이용해 행동패턴을 더 입체적으로 파악
    - scikit-learn: 군집화, 분류, 회귀
    - Pandas / NumPy: SQL에서 불러온 데이터를 변형하고 계산하는 기본 도구
- 가능한 확장 분석
    - 헤비 리뷰어 자동 분류
    - 리뷰 성향(부정적/중립적/긍정적) 기반 유저 세그먼트
    - 특정 유저가 어떤 제품군을 선호하는지 분석

##### 3. 제품·시장 구조의 시각적 해석
- SQL의 결과를 Python에서 시각화
    - Matplotlib
    - Seaborn
- 가능한 확장 시각화
    - 가격 vs 평점 구조
...
- [자료다운로드](https://github.com/j11w00n/Portfolio/blob/main/amazon_sales_data.sql/amazon_sales_dataset.sql)
